...
#define kAppId @"<YOUR CLIENT ID HERE>"
#define RESPONSE_URL (@"myAppSchema://authorize")
...
- (id) init {
    if((self = [super init])) {
        ...
        NSString* cacheFolder = [NSString stringWithFormat:@"%@cache/", DOCUMENTS_FOLDER];
...
- (NSString*) getLoginUrl {
    ...
    NSString* reqUrl = [NSString stringWithFormat:@"https://api.instagram.com/oauth/authorize/?client_id=%@&redirect_uri=%@&response_type=token&scope=" API_SCOPE "&display=touch", 
                        kAppId, responseUrl];
