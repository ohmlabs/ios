...
- (void) handleTouch:(UITouch*)touch withEvent:(UIEvent*)event
{
	...
	{				
		...
		{
			lastSelectedIndex = touchedSubview.tag;
			
			[target gallery:self didSelectColumnAtIndex:lastSelectedIndex];
		}
...
- (void) setContentOffset:(CGPoint)contentOffset
{
	if (self.pagingEnabled)
	{
		...
		{		
			...
			contentOffset.x = floorf(currentPage) * self.bounds.size.width;
...
- (NSInteger) selectedIndex
{
	if (self.pagingEnabled)
	{
		// When paging is enabled, the center cell is the view we want to select.
		
		// Note: the center property of a view is in frame coordinates. We want to hit test in bounds coordinates
		// to find the cell in the center of the bounds.
	...
	{
		// We're not paging. Just remember the last user selected index, or 0 if there isn't one.
		
		if ((-1 == lastSelectedIndex) && [[self subviews] count])
		{
			lastSelectedIndex = 0;
		}
...
{
	UIView* selectedView = [self viewWithTag:selectedIndex];
	
	if (selectedView)
	{
		lastSelectedIndex = selectedView.tag;
