...
@interface VaultArtistIndexView : UIView {
...
@private
	...
	unichar selectedChar; // We use the unichar UNICODE type to allow localization to non-Roman scripts.
	
	NSArray* sectionIndexChars;
}

@property (assign) unichar selectedChar;

// action will be called on target when the user touches this view.
// Note: by UIKit convention, the target is NOT retained.
